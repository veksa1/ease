  ...
  useEffect(() => {
    ...
    sqliteService.getTimelineEntries(today).then(entries => {
      if (entries.length > 0) {
        ...
        setDbData(latest.data);
      }
    });

    // Fetch baseline HRV (try multiple keys)
    (async () => {
      const baseline =
        (await sqliteService.getSetting('baseline_hrv')) ??
        (await sqliteService.getSetting('hrv_baseline')) ??
        (await sqliteService.getSetting('baselineHRV'));
      if (baseline != null) {
        const num = typeof baseline === 'string' ? parseFloat(baseline) : (baseline as number);
        if (Number.isFinite(num)) setBaselineHRV(num);
      }
    })();
  }, []);
